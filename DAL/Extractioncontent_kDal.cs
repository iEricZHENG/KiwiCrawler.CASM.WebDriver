...
{
	/// <summary>
	/// 数据访问类:Extractioncontent_kDal
	/// </summary>
	public partial class Extractioncontent_kDal
	{
		public Extractioncontent_kDal()
		{}
		#region  BasicMethod
		...
		public bool Add(KiwiCrawler.Model.Extractioncontent_k model)
		{
			...
			strSql.Append("insert into extractioncontent_k(");
			strSql.Append("KId,KUrl,KTitle,KPublishDateTime,KContent,KAddressBusinessType,KKeyword,KExtracionType,KExtra)");
			...
			MySqlParameter[] parameters = {
					...
					new MySqlParameter("@KExtra", MySqlDbType.Text)};
			...
			parameters[7].Value = model.KExtracionType;
			parameters[8].Value = model.KExtra;
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update extractioncontent_k set ");
		...
		public bool Delete()
		{
			//该表无主键信息，请自定义主键/条件字段
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from extractioncontent_k ");
			strSql.Append(" where ");
			MySqlParameter[] parameters = {
			};
		...
		public KiwiCrawler.Model.Extractioncontent_k GetModel()
		{
			//该表无主键信息，请自定义主键/条件字段
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select KId,KUrl,KTitle,KPublishDateTime,KContent,KAddressBusinessType,KKeyword,KExtracionType,KExtra from extractioncontent_k ");
			strSql.Append(" where ");
			MySqlParameter[] parameters = {
			};

			KiwiCrawler.Model.Extractioncontent_k model=new KiwiCrawler.Model.Extractioncontent_k();
		...
		/// 得到一个对象实体
		/// </summary>
		public KiwiCrawler.Model.Extractioncontent_k DataRowToModel(DataRow row)
		{
			KiwiCrawler.Model.Extractioncontent_k model=new KiwiCrawler.Model.Extractioncontent_k();
			if (row != null)
			{
				...
				if(row["KExtra"]!=null)
				{
					model.KExtra=row["KExtra"].ToString();
				}
			}
			return model;
		}
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select KId,KUrl,KTitle,KPublishDateTime,KContent,KAddressBusinessType,KKeyword,KExtracionType,KExtra ");
			strSql.Append(" FROM extractioncontent_k ");
			if(strWhere.Trim()!="")
			{
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(1) FROM extractioncontent_k ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
			}
		...
		public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
		{
			...
			strSql.Append(")AS Row, T.*  from extractioncontent_k T ");
		...
		public DataSet GetList(int PageSize,int PageIndex,string strWhere)
		{
			...
			parameters[0].Value = "extractioncontent_k";
			parameters[1].Value = "";
